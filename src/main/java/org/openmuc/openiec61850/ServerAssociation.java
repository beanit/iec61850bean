...
import org.openmuc.openiec61850.internal.mms.asn1.DataAccessError;
import org.openmuc.openiec61850.internal.mms.asn1.DefineNamedVariableListRequest;
import org.openmuc.openiec61850.internal.mms.asn1.DefineNamedVariableListResponse;
...
import org.openmuc.openiec61850.internal.mms.asn1.GetNameListResponse.ListOfIdentifier;
...
import org.openmuc.openiec61850.internal.mms.asn1.ReadResponse.ListOfAccessResult;
...
final class ServerAssociation {
    ...
    private MMSpdu constructAssociationResponsePdu(InitiateRequestPDU associationRequestMMSpdu) {
        ...
        initRespDetail.setNegotiatedParameterCBB(new ParameterSupportOptions(negotiatedParameterCbbBitString,
                negotiatedParameterCbbBitString.length * 8 - 5));
        initRespDetail.setServicesSupportedCalled(new ServiceSupportOptions(servicesSupportedCalledBitString,
                servicesSupportedCalledBitString.length * 8 - 3));
            ...
            try {
                ...
                if (confirmedServiceRequest.getGetNameList() != null) {
                    ...
                    if (basicObjectClass == 9) {
                        ...
                        throw new ServiceError(ServiceError.FAILED_DUE_TO_SERVER_CONSTRAINT,
                                "Unable to handle Get directory request for basic object class: " + basicObjectClass);
                    }
    ...
    private MMSpdu listenForMmsRequest(AcseAssociation acseAssociation) {
        ...
        while (true) {
            ...
            try {
                ...
                logger.warn("IOException at lower layers while listening for incoming request. Closing association.",
                        e);
                return null;
    ...
    private MMSpdu createServiceErrorResponse(ServiceError e, int invokeId) {
        ...
        asn1ServiceError = new org.openmuc.openiec61850.internal.mms.asn1.ServiceError();
    ...
    private void writeAndFillMissingWriteResults(List<WriteResponse.CHOICE> mmsResponseValues,
                    ...
                    if (mmsResponseIterator.next() == null) {
                        for (int j = 0; j < numBdas[i]; j++) {
                            ...
                            if (serviceError != null) {
                                WriteResponse.CHOICE writeResponseChoice = new WriteResponse.CHOICE();
                                writeResponseChoice
                                        .setFailure(new DataAccessError(serviceErrorToMmsError(serviceError)));
    ...
    private void getFirstWriteResults(List<WriteResponse.CHOICE> mmsResponseValues,
            List<BasicDataAttribute> totalBdasToBeWritten, int[] numBdas, int i, FcModelNode fcModelNode,
            Data mmsData) {
        ...
        if (fc == Fc.CO) {
            ...
            synchronized (urcb) {
                if (nodeName.equals("RptEna")) {
                    ...
                    if ((urcb.reserved == null || urcb.reserved == this) && !urcb.enabled) {
                        if (!((BdaOptFlds) modelNode).isBufferOverflow() && !((BdaOptFlds) modelNode).isConfigRevision()
